...
test "Vec2i arithmetic matches scalar reference" {
    ...
    try testing.expectEqual(@as(i32, 13), sum[0]);
    try testing.expectEqual(@as(i32, 15), sum[1]);

    const prod = Vec2i.mul(a, b).toArray();
    try testing.expectEqual(@as(i32, 30), prod[0]);
    try testing.expectEqual(@as(i32, -100), prod[1]);
}
...
test "Vec2i sign, comparison, select, and lane access" {
    ...
    try testing.expectEqual(@as(i32, -10), diff[0]);
    try testing.expectEqual(@as(i32, 19), diff[1]);
    ...
    try testing.expectEqual(@as(i32, 7), abs_vals[0]);
    try testing.expectEqual(@as(i32, 9), abs_vals[1]);

    const mins = Vec2i.min(a, b).toArray();
    const maxs = Vec2i.max(a, b).toArray();
    try testing.expectEqual(@as(i32, -7), mins[0]);
    try testing.expectEqual(@as(i32, 3), maxs[0]);
    try testing.expectEqual(@as(i32, -10), mins[1]);
    try testing.expectEqual(@as(i32, 9), maxs[1]);

    const selected = Vec2i.select(Mask2.fromBits(0b01), a, b).toArray();
    try testing.expectEqual(@as(i32, -7), selected[0]);
    try testing.expectEqual(@as(i32, -10), selected[1]);

    var lanes = Vec2i.splat(0);
    lanes = lanes.insert(0, 11);
    lanes = lanes.insert(1, -12);
    try testing.expectEqual(@as(i32, 11), lanes.extract(0));
    try testing.expectEqual(@as(i32, -12), lanes.extract(1));
}

test "Vec2i shift and bit-not operations" {
    const v = Vec2i.init(.{ 1, -8 });
    const left = Vec2i.shl(v, 2).toArray();
    try testing.expectEqual(@as(i32, 4), left[0]);
    try testing.expectEqual(@as(i32, -32), left[1]);

    const right = Vec2i.shr(v, 1).toArray();
    try testing.expectEqual(@as(i32, 0), right[0]);
    try testing.expectEqual(@as(i32, -4), right[1]);

    const not_v = Vec2i.bitNot(v).toArray();
    try testing.expectEqual(@as(i32, ~@as(i32, 1)), not_v[0]);
    try testing.expectEqual(@as(i32, ~@as(i32, -8)), not_v[1]);
}
