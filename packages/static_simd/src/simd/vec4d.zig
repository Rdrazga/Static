...
test "Vec4d arithmetic matches scalar reference" {
    ...
    try testing.expectEqual(@as(f64, 22.0), sum[1]);
    try testing.expectEqual(@as(f64, 33.0), sum[2]);
    try testing.expectEqual(@as(f64, 44.0), sum[3]);

    const prod = Vec4d.mul(a, b).toArray();
    try testing.expectEqual(@as(f64, 10.0), prod[0]);
    try testing.expectEqual(@as(f64, 40.0), prod[1]);
    try testing.expectEqual(@as(f64, 90.0), prod[2]);
    try testing.expectEqual(@as(f64, 160.0), prod[3]);
}
...
test "Vec4d precision check — f64 vs f32" {
    ...
    try testing.expect(sum_d[0] != 1.0);

    // The same value in f32 collapses to 1.0.
    const tiny_f32: f32 = @floatCast(tiny);
    try testing.expectEqual(@as(f32, 1.0), 1.0 + tiny_f32);
}
...
test "Vec4d arithmetic, sign, and comparison helpers" {
    ...
    try testing.expectEqual(@as(f64, 6.0), diff[0]);
    try testing.expectEqual(@as(f64, -3.0), diff[1]);
    try testing.expectEqual(@as(f64, 3.0), diff[2]);
    try testing.expectEqual(@as(f64, 2.0), diff[3]);

    const quot = Vec4d.div(a, b).toArray();
    try testing.expectEqual(@as(f64, 4.0), quot[0]);
    try testing.expectEqual(@as(f64, 2.0), quot[1]);
    try testing.expectEqual(@as(f64, 4.0), quot[2]);
    try testing.expectEqual(@as(f64, 0.5), quot[3]);

    const neg = Vec4d.negate(a).toArray();
    try testing.expectEqual(@as(f64, -8.0), neg[0]);
    try testing.expectEqual(@as(f64, 6.0), neg[1]);

    const abs_vals = Vec4d.abs(a).toArray();
    try testing.expectEqual(@as(f64, 8.0), abs_vals[0]);
    try testing.expectEqual(@as(f64, 6.0), abs_vals[1]);

    const mins = Vec4d.min(a, b).toArray();
    const maxs = Vec4d.max(a, b).toArray();
    try testing.expectEqual(@as(f64, 2.0), mins[0]);
    try testing.expectEqual(@as(f64, 8.0), maxs[0]);
    try testing.expectEqual(@as(f64, -6.0), mins[1]);
    try testing.expectEqual(@as(f64, -3.0), maxs[1]);
}
...
test "Vec4d copySign, select, and lane access" {
    ...
    try testing.expectEqual(@as(f64, -1.0), signed[0]);
    try testing.expectEqual(@as(f64, 2.0), signed[1]);
    try testing.expectEqual(@as(f64, -3.0), signed[2]);
    try testing.expectEqual(@as(f64, 4.0), signed[3]);
    ...
    try testing.expectEqual(@as(f64, 10.0), selected[0]);
    try testing.expectEqual(@as(f64, 2.0), selected[1]);
    try testing.expectEqual(@as(f64, 30.0), selected[2]);
    try testing.expectEqual(@as(f64, 4.0), selected[3]);

    var lanes = Vec4d.splat(0.0);
    lanes = lanes.insert(0, 7.0);
    lanes = lanes.insert(3, -9.0);
    try testing.expectEqual(@as(f64, 7.0), lanes.extract(0));
    try testing.expectEqual(@as(f64, -9.0), lanes.extract(3));
}
