...
test "Vec8i basic arithmetic" {
    ...
    for (diff) |val| {
        try std.testing.expectEqual(@as(i32, 4), val);
    }

    const prod = Vec8i.mul(a, b).toArray();
    for (prod) |val| {
        try std.testing.expectEqual(@as(i32, 21), val);
    }
}
...
test "Vec8i bitwise operations" {
    ...
    try std.testing.expectEqual(@as(i32, 0b1000), and_result[7]);
...
test "Vec8i shifts and roundtrip" {
    ...
    for (left) |val| {
        try std.testing.expectEqual(@as(i32, 8), val);
    }
...
test "Vec8i sign, comparison, and select helpers" {
    ...
    try std.testing.expectEqual(@as(i32, 8), neg[0]);
    try std.testing.expectEqual(@as(i32, -8), neg[7]);

    const abs_vals = Vec8i.abs(a).toArray();
    try std.testing.expectEqual(@as(i32, 8), abs_vals[0]);
    try std.testing.expectEqual(@as(i32, 8), abs_vals[7]);

    const mins = Vec8i.min(a, b).toArray();
    const maxs = Vec8i.max(a, b).toArray();
    try std.testing.expectEqual(@as(i32, -8), mins[0]);
    try std.testing.expectEqual(@as(i32, -7), maxs[0]);
    try std.testing.expectEqual(@as(i32, 6), mins[6]);
    try std.testing.expectEqual(@as(i32, 7), maxs[6]);

    const selected = Vec8i.select(Mask8.fromBits(0b01010101), a, b).toArray();
    try std.testing.expectEqual(@as(i32, -8), selected[0]);
    try std.testing.expectEqual(@as(i32, -6), selected[1]);
    try std.testing.expectEqual(@as(i32, 6), selected[6]);
    try std.testing.expectEqual(@as(i32, 9), selected[7]);
}

test "Vec8i bit-not and lane access" {
    const v = Vec8i.fromArray(.{ 0, 1, 2, 3, 4, 5, 6, 7 });
    const not_v = Vec8i.bitNot(v).toArray();
    try std.testing.expectEqual(@as(i32, -1), not_v[0]);
    try std.testing.expectEqual(@as(i32, ~@as(i32, 7)), not_v[7]);

    const shifted = Vec8i.shr(Vec8i.fromArray(.{ -16, -8, -4, -2, 2, 4, 8, 16 }), 1).toArray();
    try std.testing.expectEqual(@as(i32, -8), shifted[0]);
    try std.testing.expectEqual(@as(i32, 8), shifted[7]);

    var lanes = Vec8i.splat(0);
    lanes = lanes.insert(0, 17);
    lanes = lanes.insert(7, -18);
    try std.testing.expectEqual(@as(i32, 17), lanes.extract(0));
    try std.testing.expectEqual(@as(i32, -18), lanes.extract(7));
}
