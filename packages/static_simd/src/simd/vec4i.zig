...
test "Vec4i arithmetic matches scalar reference" {
    ...
    try std.testing.expectEqual(@as(i32, 10), prod[0]);
    try std.testing.expectEqual(@as(i32, 40), prod[1]);
    try std.testing.expectEqual(@as(i32, 90), prod[2]);
    try std.testing.expectEqual(@as(i32, 160), prod[3]);
}
...
test "Vec4i bitwise operations" {
    ...
    try std.testing.expectEqual(@as(i32, -1), not_result[0]);
}
...
test "Vec4i sign, comparison, select, and lane helpers" {
    ...
    try std.testing.expectEqual(@as(i32, -6), diff[0]);
    try std.testing.expectEqual(@as(i32, 8), diff[1]);
    try std.testing.expectEqual(@as(i32, 0), diff[2]);
    try std.testing.expectEqual(@as(i32, -1), diff[3]);

    const neg = Vec4i.negate(a).toArray();
    try std.testing.expectEqual(@as(i32, 5), neg[0]);
    try std.testing.expectEqual(@as(i32, -6), neg[1]);
    try std.testing.expectEqual(@as(i32, 7), neg[2]);
    try std.testing.expectEqual(@as(i32, -8), neg[3]);

    const abs_vals = Vec4i.abs(a).toArray();
    try std.testing.expectEqual(@as(i32, 5), abs_vals[0]);
    try std.testing.expectEqual(@as(i32, 6), abs_vals[1]);
    try std.testing.expectEqual(@as(i32, 7), abs_vals[2]);
    try std.testing.expectEqual(@as(i32, 8), abs_vals[3]);

    const mins = Vec4i.min(a, b).toArray();
    const maxs = Vec4i.max(a, b).toArray();
    try std.testing.expectEqual(@as(i32, -5), mins[0]);
    try std.testing.expectEqual(@as(i32, 1), maxs[0]);
    try std.testing.expectEqual(@as(i32, -2), mins[1]);
    try std.testing.expectEqual(@as(i32, 6), maxs[1]);

    const selected = Vec4i.select(Mask4.fromBits(0b0101), a, b).toArray();
    try std.testing.expectEqual(@as(i32, -5), selected[0]);
    try std.testing.expectEqual(@as(i32, -2), selected[1]);
    try std.testing.expectEqual(@as(i32, -7), selected[2]);
    try std.testing.expectEqual(@as(i32, 9), selected[3]);

    var lanes = Vec4i.splat(0);
    lanes = lanes.insert(0, 12);
    lanes = lanes.insert(3, -13);
    try std.testing.expectEqual(@as(i32, 12), lanes.extract(0));
    try std.testing.expectEqual(@as(i32, -13), lanes.extract(3));
}
