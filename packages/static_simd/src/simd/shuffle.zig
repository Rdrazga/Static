...
test "shuffle4f basic reorder" {
    ...
    try testing.expectEqual(@as(f32, 40.0), result[0]);
...
test "shuffle4f supports zero fill with negative indices" {
    ...
    try testing.expectEqual(@as(f32, 30.0), result[0]);
        ...
        inline for (0..4) |b| {
            inline for (0..4) |c| {
                inline for (0..4) |d| {
                    ...
                    try testing.expectEqual(@as(f32, @floatFromInt(a)), result[0]);
                    try testing.expectEqual(@as(f32, @floatFromInt(b)), result[1]);
                    try testing.expectEqual(@as(f32, @floatFromInt(c)), result[2]);
                    try testing.expectEqual(@as(f32, @floatFromInt(d)), result[3]);
                }
            }
        }
    }
...
test "shuffle2x4f two-source merge" {
    ...
    try testing.expectEqual(@as(f32, 6.0), result[3]);
}

test "reverse4f" {
    const v = vec4f.Vec4f.init(.{ 1.0, 2.0, 3.0, 4.0 });
    const r = reverse4f(v).toArray();
    try testing.expectEqual(@as(f32, 4.0), r[0]);
    try testing.expectEqual(@as(f32, 1.0), r[3]);
}
...
test "unpackLo4f and unpackHi4f interleave lanes" {
    ...
    try testing.expectEqualSlices(f32, &.{ 1.0, 5.0, 2.0, 6.0 }, lo[0..]);
    try testing.expectEqualSlices(f32, &.{ 3.0, 7.0, 4.0, 8.0 }, hi[0..]);
}
...
test "broadcast4f" {
    ...
    try testing.expectEqual(@as(f32, 30.0), b[0]);
    try testing.expectEqual(@as(f32, 30.0), b[3]);
}
...
test "shuffle8f and helpers" {
    ...
    try testing.expectEqual(@as(f32, 80.0), shuffled[0]);
    try testing.expectEqual(@as(f32, 10.0), shuffled[7]);

    const zero_filled = shuffle8f(v, .{ 0, -1, 1, -1, 2, -1, 3, -1 }).toArray();
    try testing.expectEqual(@as(f32, 10.0), zero_filled[0]);
    try testing.expectEqual(@as(f32, 0.0), zero_filled[1]);
    try testing.expectEqual(@as(f32, 40.0), zero_filled[6]);
    try testing.expectEqual(@as(f32, 0.0), zero_filled[7]);

    const reversed = reverse8f(v).toArray();
    try testing.expectEqual(@as(f32, 80.0), reversed[0]);
    try testing.expectEqual(@as(f32, 10.0), reversed[7]);
    ...
    try testing.expectEqual(@as(f32, 50.0), broadcasted[0]);
    try testing.expectEqual(@as(f32, 50.0), broadcasted[7]);
}
...
test "unpackLo8f and unpackHi8f interleave halves" {
    ...
    try testing.expectEqualSlices(f32, &.{ 1, 10, 2, 20, 3, 30, 4, 40 }, lo[0..]);
    try testing.expectEqualSlices(f32, &.{ 5, 50, 6, 60, 7, 70, 8, 80 }, hi[0..]);
}

test "Swizzle4 predefined patterns" {
    const v = vec4f.Vec4f.init(.{ 1.0, 2.0, 3.0, 4.0 });
    const xxxx = Swizzle4.xxxx(v).toArray();
    try testing.expectEqualSlices(f32, &.{ 1, 1, 1, 1 }, xxxx[0..]);
    const xyzw = Swizzle4.xyzw(v).toArray();
    try testing.expectEqualSlices(f32, &.{ 1, 2, 3, 4 }, xyzw[0..]);
    const xyxy = Swizzle4.xyxy(v).toArray();
    try testing.expectEqualSlices(f32, &.{ 1, 2, 1, 2 }, xyxy[0..]);
    const zwzw = Swizzle4.zwzw(v).toArray();
    try testing.expectEqualSlices(f32, &.{ 3, 4, 3, 4 }, zwzw[0..]);
}
