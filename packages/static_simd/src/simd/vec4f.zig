...
test "Vec4f arithmetic matches scalar reference" {
    ...
    try testing.expectEqual(@as(f32, 33.0), sum[2]);
    try testing.expectEqual(@as(f32, 44.0), sum[3]);

    const prod = Vec4f.mul(a, b).toArray();
    try testing.expectEqual(@as(f32, 10.0), prod[0]);
    try testing.expectEqual(@as(f32, 40.0), prod[1]);
    try testing.expectEqual(@as(f32, 90.0), prod[2]);
    try testing.expectEqual(@as(f32, 160.0), prod[3]);
}
...
test "Vec4f sign operations" {
    ...
    try testing.expectEqual(@as(f32, 3.0), a[2]);
}

test "Vec4f lane extract and insert" {
    const v = Vec4f.init(.{ 10.0, 20.0, 30.0, 40.0 });
    try testing.expectEqual(@as(f32, 30.0), v.extract(2));

    const v2 = v.insert(1, 99.0);
    try testing.expectEqual(@as(f32, 99.0), v2.extract(1));
    try testing.expectEqual(@as(f32, 10.0), v2.extract(0));
}
...
test "Vec4f NaN propagation (mandatory)" {
    ...
    try testing.expect(std.math.isNan(sum[1]));
...
test "Vec4f div by zero produces Inf" {
    ...
    try testing.expect(std.math.isInf(result[0]));
    try testing.expect(!std.math.signbit(result[0]));
    try testing.expect(std.math.isInf(result[1]));
    try testing.expect(std.math.signbit(result[1]));
    try testing.expect(std.math.isInf(result[2]));
    try testing.expect(std.math.signbit(result[2]));
    try testing.expect(std.math.isInf(result[3]));
    try testing.expect(!std.math.signbit(result[3]));
}
...
test "Vec4f comparison, select, and copySign helpers" {
    ...
    try testing.expectEqual(@as(f32, 4.0), diff[0]);
    try testing.expectEqual(@as(f32, 1.0), diff[1]);
    try testing.expectEqual(@as(f32, -1.0), diff[2]);
    try testing.expectEqual(@as(f32, -5.0), diff[3]);

    const mins = Vec4f.min(a, b).toArray();
    const maxs = Vec4f.max(a, b).toArray();
    try testing.expectEqual(@as(f32, 1.0), mins[0]);
    try testing.expectEqual(@as(f32, 5.0), maxs[0]);
    ...
    try testing.expectEqual(@as(f32, 5.0), selected[0]);
    try testing.expectEqual(@as(f32, -3.0), selected[1]);
    try testing.expectEqual(@as(f32, 7.0), selected[2]);
    try testing.expectEqual(@as(f32, -4.0), selected[3]);

    const copy_sign = Vec4f.copySign(Vec4f.splat(2.0), a).toArray();
    try testing.expectEqual(@as(f32, 2.0), copy_sign[0]);
    try testing.expectEqual(@as(f32, -2.0), copy_sign[1]);
    try testing.expectEqual(@as(f32, 2.0), copy_sign[2]);
    try testing.expectEqual(@as(f32, -2.0), copy_sign[3]);
}
