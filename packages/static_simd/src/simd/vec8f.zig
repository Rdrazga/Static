...
test "Vec8f NaN propagation" {
    ...
    try testing.expect(std.math.isNan(sum[0]));
    try testing.expect(std.math.isNan(sum[7]));

    const inf_vec = Vec8f.splat(std.math.inf(f32));
    const zero = Vec8f.splat(0.0);
    const prod = Vec8f.mul(inf_vec, zero).toArray();
    try testing.expect(std.math.isNan(prod[0]));
}
...
test "Vec8f div and helper operations" {
    ...
    try testing.expectApproxEqAbs(@as(f32, 4.0), quot[0], 1.0e-6);
    try testing.expectApproxEqAbs(@as(f32, 2.0), quot[1], 1.0e-6);
    try testing.expectApproxEqAbs(@as(f32, 4.0), quot[2], 1.0e-6);
    try testing.expectApproxEqAbs(@as(f32, 0.5), quot[3], 1.0e-6);
    try testing.expectApproxEqAbs(@as(f32, 2.0), quot[4], 1.0e-6);
    try testing.expectApproxEqAbs(@as(f32, 2.0), quot[5], 1.0e-6);
    try testing.expectApproxEqAbs(@as(f32, 2.0), quot[6], 1.0e-6);
    try testing.expectApproxEqAbs(@as(f32, 2.0), quot[7], 1.0e-6);

    const neg = Vec8f.negate(a).toArray();
    try testing.expectEqual(@as(f32, -8.0), neg[0]);
    try testing.expectEqual(@as(f32, 16.0), neg[7]);

    const abs_vals = Vec8f.abs(a).toArray();
    try testing.expectEqual(@as(f32, 8.0), abs_vals[0]);
    try testing.expectEqual(@as(f32, 16.0), abs_vals[7]);

    const mins = Vec8f.min(a, b).toArray();
    const maxs = Vec8f.max(a, b).toArray();
    try testing.expectEqual(@as(f32, 2.0), mins[0]);
    try testing.expectEqual(@as(f32, 8.0), maxs[0]);
    try testing.expectEqual(@as(f32, -16.0), mins[7]);
    try testing.expectEqual(@as(f32, -8.0), maxs[7]);
}
...
test "Vec8f copySign, select, and lane access" {
    ...
    try testing.expectEqual(@as(f32, 2.5), signed[0]);
    try testing.expectEqual(@as(f32, -2.5), signed[1]);
    try testing.expectEqual(@as(f32, -2.5), signed[7]);
    ...
    try testing.expectEqual(@as(f32, 0.0), selected[0]);
    try testing.expectEqual(@as(f32, 99.0), selected[1]);
    try testing.expectEqual(@as(f32, 6.0), selected[6]);
    try testing.expectEqual(@as(f32, 99.0), selected[7]);

    var lanes = Vec8f.splat(0.0);
    lanes = lanes.insert(0, 3.0);
    lanes = lanes.insert(7, 4.0);
    try testing.expectEqual(@as(f32, 3.0), lanes.extract(0));
    try testing.expectEqual(@as(f32, 4.0), lanes.extract(7));
}
