...
pub fn main() void {
    ...
    assert(@abs(sin_values[1] - 0.5) <= tolerance);
    assert(@abs(cos_values[2] - (@sqrt(@as(f32, 2.0)) * 0.5)) <= tolerance);
    assert(@abs(tan_values[3] - 1.7320508) <= 3.0e-6);

    const scalar_sin = simd.trig.sin_scalar(std.math.pi / 6.0);
    assert(@abs(scalar_sin - sin_values[1]) <= tolerance);
}
