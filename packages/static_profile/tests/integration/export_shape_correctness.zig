...
test "EnabledTrace exports an interleaved B/C/B/C/E/E trace exactly" {
    var trace = try profile.trace.EnabledTrace.init(testing.allocator, 6);
    defer trace.deinit();
    ...
    try testing.expectEqual(@as(usize, 6), trace.events.items.len);
    try testing.expectEqual(@as(u32, 0), trace.zone_depth);
