...
const CounterSink = struct {
    ...
    fn emit(self: *@This(), name: []const u8, value: i64) void {
        self.trace.recordCounter(name, self.next_ts, self.tid, value) catch |err| {
            panic("unexpected trace error: {s}", .{@errorName(err)});
        };
        self.next_ts += 1;
    }
};
...
test "hooks.emitCounters preserves declared order when wired into EnabledTrace" {
    var trace = try profile.trace.EnabledTrace.init(testing.allocator, 3);
    defer trace.deinit();
    ...
    try testing.expectEqual(@as(usize, 3), trace.events.items.len);
    try testing.expectEqualStrings("cpu.idle", trace.events.items[0].counter.name);
    try testing.expectEqualStrings("cpu.busy", trace.events.items[1].counter.name);
    try testing.expectEqualStrings("cpu.steal", trace.events.items[2].counter.name);
    try testing.expectEqual(@as(u64, 100), trace.events.items[0].counter.ts);
    try testing.expectEqual(@as(u64, 101), trace.events.items[1].counter.ts);
    try testing.expectEqual(@as(u64, 102), trace.events.items[2].counter.ts);
...
test "repeated same-name counter updates remain distinct events" {
    var trace = try profile.trace.EnabledTrace.init(testing.allocator, 4);
    defer trace.deinit();
    ...
    try testing.expectEqual(@as(usize, 3), trace.events.items.len);
    try testing.expectEqualStrings("queue_depth", trace.events.items[0].counter.name);
    try testing.expectEqualStrings("queue_depth", trace.events.items[1].counter.name);
    try testing.expectEqualStrings("queue_depth", trace.events.items[2].counter.name);
    try testing.expectEqual(@as(i64, 10), trace.events.items[0].counter.value);
    try testing.expectEqual(@as(i64, 11), trace.events.items[1].counter.value);
    try testing.expectEqual(@as(i64, 12), trace.events.items[2].counter.value);
...
test "NoSpaceLeft on the last counter append does not corrupt prior export state" {
    var trace = try profile.trace.EnabledTrace.init(testing.allocator, 2);
    defer trace.deinit();

    try trace.recordCounter("first", 1, 4, 1);
    try trace.recordCounter("second", 2, 4, 2);

    var aw_before: std.Io.Writer.Allocating = .init(testing.allocator);
    ...
    try testing.expectError(profile.trace.Error.NoSpaceLeft, trace.recordCounter("third", 3, 4, 3));
    try testing.expectEqual(@as(usize, 2), trace.events.items.len);

    var aw_after: std.Io.Writer.Allocating = .init(testing.allocator);
...
test "NoSpaceLeft on endZone preserves open depth and prior export state" {
    var trace = try profile.trace.EnabledTrace.init(testing.allocator, 2);
    defer {
        trace.events.deinit(testing.allocator);
        trace = undefined;
    }
    ...
    try testing.expectError(profile.trace.Error.NoSpaceLeft, trace.endZone(tok, 12));
    try testing.expectEqual(@as(u32, 1), trace.zone_depth);
    try testing.expectEqual(@as(usize, 2), trace.events.items.len);
