...
pub fn current() BuildOptions {
    ...
    assert(result.single_threaded == build_options.single_threaded);
    assert(result.enable_os_backends == build_options.enable_os_backends);
    assert(result.enable_tracing == build_options.enable_tracing);
    return result;
}

test "canonical option names and values are exposed" {
    const opts = current();
    try testing.expectEqual(opts.single_threaded, build_options.single_threaded);
    try testing.expectEqual(opts.enable_os_backends, build_options.enable_os_backends);
    try testing.expectEqual(opts.enable_tracing, build_options.enable_tracing);
    try testing.expectEqualStrings("single_threaded", OptionNames.single_threaded);
    try testing.expectEqualStrings("enable_os_backends", OptionNames.enable_os_backends);
    try testing.expectEqualStrings("enable_tracing", OptionNames.enable_tracing);
}
