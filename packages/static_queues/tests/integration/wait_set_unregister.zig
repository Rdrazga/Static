...
test "wait set unregister removes the source from selection" {
    ...
    var c1 = try Channel.init(std.testing.allocator, .{ .capacity = 1 });
    defer c1.deinit();
    var c2 = try Channel.init(std.testing.allocator, .{ .capacity = 1 });
    defer c2.deinit();
    ...
    try std.testing.expectError(error.InvalidIndex, wait_set.unregister(2));
    try std.testing.expectError(error.InvalidIndex, wait_set.unregister(3));
