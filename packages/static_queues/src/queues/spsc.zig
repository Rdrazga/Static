...
pub fn SpscQueue(comptime T: type) type {
    ...
    return struct {
        ...
        pub fn init(allocator: std.mem.Allocator, cfg: Config) Error!Self {
            ...
            assert(self.buf.len == cfg.capacity + 1);
        ...
        pub fn deinit(self: *Self) void {
            ...
            assert(self.head.load(.monotonic) < self.buf.len);
            assert(self.tail.load(.monotonic) < self.buf.len);
            assert(!self.send_reserved);
            assert(!self.recv_reserved);
        ...
        fn nextIndex(index: usize, capacity_slots: usize) usize {
            ...
            assert(index < capacity_slots);
            // Precondition: capacity must be non-zero.
            assert(capacity_slots > 0);
            const next = index + 1;
...
test "spsc queue wraparound and WouldBlock semantics" {
    var q = try SpscQueue(u8).init(testing.allocator, .{ .capacity = 2 });
    defer q.deinit();
...
test "spsc queue introspection methods reflect queue state" {
    var q = try SpscQueue(u8).init(testing.allocator, .{ .capacity = 3 });
    defer q.deinit();

    try testing.expectEqual(@as(usize, 3), q.capacity());
...
test "spsc queue reserve and commit send path provides zero-copy write" {
    ...
    try testing.expectEqual(@as(u8, 42), try q.tryRecv());
}
...
test "spsc queue reserve and commit recv path provides zero-copy read" {
    ...
    try testing.expectEqual(@as(u8, 77), slot.*);
    q.commitRecv();
    try testing.expect(q.isEmpty());
}
...
test "spsc queue reservation abort restores regular send and recv paths" {
    ...
    try testing.expectEqual(@as(u8, 10), recv_slot.*);
