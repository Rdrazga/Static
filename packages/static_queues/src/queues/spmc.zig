//! SpmcQueue: single-producer, multi-consumer lock-free bounded queue.
//!
//! Capacity: fixed at init time; must be a power of two >= 2.
//! Thread safety: single producer and multiple concurrent consumers.
...
pub fn SpmcQueue(comptime T: type) type {
    ...
    return struct {
        ...
        pub const concurrency: contracts.Concurrency = .spmc;
        ...
        pub fn trySend(self: *Self, value: T) TrySendError!void {
            ...
            if (dif == 0) {
                ...
                // This branch indicates the single-producer contract has been violated.
                @panic("spmc trySend reached invalid sequence state; single-producer contract violated");
            }
        }
