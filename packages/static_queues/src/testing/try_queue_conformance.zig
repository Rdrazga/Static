...
pub fn runTryQueueConformance(comptime Q: type, allocator: std.mem.Allocator, capacity: usize) !void {
    ...
    try testing.expectEqual(capacity, queue.capacity());
    ...
    try testing.expectError(error.WouldBlock, queue.trySend(@as(T, @intCast(capacity + 1))));

    i = 0;
    while (i < capacity) : (i += 1) {
        const expected: T = @as(T, @intCast(i + 1));
        try testing.expectEqual(expected, try queue.tryRecv());
        try testing.expect(queue.len() <= queue.capacity());
    }
