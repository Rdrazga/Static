...
pub fn runChannelConformance(comptime C: type, allocator: std.mem.Allocator, capacity: usize) !void {
    ...
    try std.testing.expectError(error.WouldBlock, channel.trySend(@as(C.Element, 3)));

    try std.testing.expectEqual(@as(C.Element, 1), try channel.tryRecv());
    channel.close();

    try std.testing.expectError(error.Closed, channel.trySend(@as(C.Element, 9)));
    try std.testing.expectEqual(@as(C.Element, 2), try channel.tryRecv());
    ...
    if (@hasDecl(C, "trySendBatch") and @hasDecl(C, "tryRecvBatch")) {
        ...
        if (@hasDecl(C, "ChannelBatchOptions") and @hasDecl(C, "trySendBatchWith") and @hasDecl(C, "tryRecvBatchWith")) {
            ...
            try std.testing.expectEqual(@as(usize, 0), empty_send_closed);

            const empty_recv_closed = try batch_channel.tryRecvBatchWith(out[0..0], .{
                .items_max = 1,
                .wake_mode = wake_mode_close,
            });
            try std.testing.expectEqual(@as(usize, 0), empty_recv_closed);

            try std.testing.expectError(error.Closed, batch_channel.trySendBatchWith(&batch_values, .{
                .items_max = 1,
                .wake_mode = wake_mode_batch,
            }));
    ...
    if (C.supports_blocking_wait) {
        ...
        try std.testing.expectError(error.Cancelled, cancellable_channel.send(@as(C.Element, 4), token));
        try std.testing.expectError(error.Cancelled, cancellable_channel.recv(token));
    }
    ...
    if (@hasDecl(C, "sendTimeout") and @hasDecl(C, "recvTimeout")) {
        ...
        try std.testing.expectError(error.Timeout, timed_channel.sendTimeout(@as(C.Element, 7), null, 0));

        _ = try timed_channel.tryRecv();
        try std.testing.expectError(error.Timeout, timed_channel.recvTimeout(null, 0));
    }
}
