...
fn runImmediateSuccess(
    ...
    try testing.expectEqual(@as(u32, 1), delivery.success_count);
    try testing.expectEqual(@as(u32, 0), delivery.failure_count);
    ...
    try testing.expectEqual(@as(u32, 201), completion.value);
    try testing.expectEqual(@as(u32, request_id), completion.request_id);
    ...
    try testing.expect((try temporal.checkNever(snapshot, .{
        .label = "retry_queue.emit",
        .surface_label = "retry_queue",
    })).check_result.passed);
    try testing.expect((try temporal.checkExactlyOnce(snapshot, .{
...
fn runFailOnceThenSuccess(
    ...
    try testing.expectEqual(@as(u32, 0), first_delivery.success_count);
    try testing.expectEqual(@as(u32, 1), first_delivery.failure_count);

    const first_completion = try completion_mailbox.recv();
    try testing.expectEqual(storage_mod.CompletionStatus.failed, first_completion.status);
    try testing.expectEqual(@as(u32, 501), first_completion.value);
    ...
    try testing.expectEqual(@as(u32, request_id), retry.request_id);
    ...
    try testing.expectEqual(@as(u32, 1), second_delivery.success_count);
    try testing.expectEqual(@as(u32, 0), second_delivery.failure_count);

    const success = try completion_mailbox.recv();
    try testing.expectEqual(storage_mod.CompletionStatus.success, success.status);
    try testing.expectEqual(@as(u32, 202), success.value);
    try testing.expectEqual(@as(u32, request_id), success.request_id);
    ...
    try testing.expect((try temporal.checkHappensBefore(snapshot, .{
...
fn runFailUntilExhausted(
    ...
    for (failure_values[0..2]) |failure_value| {
        ...
        try testing.expectEqual(@as(u32, 0), delivery.success_count);
        try testing.expectEqual(@as(u32, 1), delivery.failure_count);

        const completion = try completion_mailbox.recv();
        try testing.expectEqual(storage_mod.CompletionStatus.failed, completion.status);
        try testing.expectEqual(failure_value, completion.value);
        ...
        const retry = try retry_mailbox.recv();
        try testing.expectEqual(@as(u32, request_id), retry.request_id);
        try testing.expectEqual(current_attempt, retry.attempt);
    }
    ...
    try testing.expectEqual(@as(u32, 0), terminal_delivery.success_count);
    try testing.expectEqual(@as(u32, 1), terminal_delivery.failure_count);

    const terminal_completion = try completion_mailbox.recv();
    try testing.expectEqual(storage_mod.CompletionStatus.failed, terminal_completion.status);
    try testing.expectEqual(failure_values[2], terminal_completion.value);
    ...
    const snapshot = sim_fixture.traceBufferPtr().?.snapshot();
    try testing.expectEqual(@as(usize, 2), countTraceEvents(snapshot, "retry_queue.emit", "retry_queue"));
    try testing.expectEqual(@as(usize, 3), countTraceEvents(snapshot, "storage_lane.failed", "storage_lane"));
    try testing.expect((try temporal.checkNever(snapshot, .{
        .label = "storage_lane.success",
        .surface_label = "storage_lane",
    })).check_result.passed);
    ...
    try testing.expectEqual(@as(u32, 0), retry_mailbox.len());
}
