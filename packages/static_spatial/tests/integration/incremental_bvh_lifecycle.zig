...
test "IncrementalBVH reports total hit counts when output buffers truncate" {
    ...
    try testing.expectEqual(@as(u32, 3), aabb_count);
    try testing.expect(aabb_count > aabb_hits.len);
    for (aabb_hits) |value| {
        try testing.expect(containsValue(expected[0..], value));
    }

    var ray_hits: [1]u32 = undefined;
    const ray = Ray3.init(-5.0, 0.5, 0.5, 1.0, 0.0, 0.0);
    const ray_count = bvh.queryRay(ray, &ray_hits);
    try testing.expectEqual(@as(u32, 3), ray_count);
    try testing.expect(ray_count > ray_hits.len);
    try testing.expect(containsValue(expected[0..], ray_hits[0]));
}
...
test "IncrementalBVH lifecycle keeps inserts, refits, and removals aligned with queries" {
    ...
    try testing.expect(containsValue(ray_hits[0..ray_hit_count], 10));

    bvh.refit(first, AABB3.init(50, 50, 50, 51, 51, 51));
    const old_hit_count = bvh.queryAABB(AABB3.init(-1, -1, -1, 2, 2, 2), &buf);
    try testing.expect(!containsValue(buf[0..old_hit_count], 10));
    ...
    try testing.expect(!containsValue(ray_hits[0..moved_ray_hits], 10));

    bvh.remove(second);
    try testing.expectEqual(@as(u32, 2), bvh.count());
    const after_remove_count = bvh.queryAABB(AABB3.init(4, 4, 4, 7, 7, 7), &buf);
    try testing.expect(!containsValue(buf[0..after_remove_count], 20));

    bvh.remove(first);
    bvh.remove(third);
    try testing.expectEqual(@as(u32, 0), bvh.count());
    try testing.expectEqual(@as(u32, 0), bvh.queryAABB(AABB3.init(-100, -100, -100, 100, 100, 100), &buf));
    try testing.expectEqual(@as(u32, 0), bvh.queryRay(ray, &ray_hits));
}
...
test "IncrementalBVH can empty out and accept new inserts after removals" {
    ...
    try testing.expectEqual(@as(u32, 1), bvh.count());
    ...
    bvh.remove(handle);
    try testing.expectEqual(@as(u32, 0), bvh.count());
    try testing.expectEqual(@as(u32, 0), bvh.queryAABB(AABB3.init(0, 0, 0, 3, 3, 3), &buf));

    const reinsertion = try bvh.insert(AABB3.init(8, 8, 8, 9, 9, 9), 88);
    try testing.expect(reinsertion != BVH.INVALID);
    try testing.expectEqual(@as(u32, 1), bvh.count());
