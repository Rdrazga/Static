...
fn expectAscending(hits: []const BVH.RayHit) !void {
    ...
    for (hits[1..], 1..) |hit, index| {
        try testing.expect(hits[index - 1].t <= hit.t);
    }
}
...
test "BVH queryRaySorted reports total hits when output truncates" {
    ...
    var bvh = try BVH.build(testing.allocator, &items, .{ .max_leaf_items = 1 });
    ...
    for (truncated_hits[0..]) |hit| {
        try testing.expect(containsHit(full_hits[0..full_count], hit));
    }
}
