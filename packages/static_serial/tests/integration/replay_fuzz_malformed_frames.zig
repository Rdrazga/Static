...
const frame_parse_violation = [_]checker.Violation{
    .{
        .code = "static_serial.frame_parse_runtime",
        .message = "serial reader frame parsing diverged from the bounded reference parser",
    },
};

const retained_checksum_violation = [_]checker.Violation{
    .{
        .code = "static_serial.retained_checksum_mismatch",
...
const retained_truncated_violation = [_]checker.Violation{
    .{
        .code = "static_serial.retained_truncated_frame",
...
const retained_noncanonical_violation = [_]checker.Violation{
    .{
        .code = "static_serial.retained_noncanonical_prefix",
        .message = "retained non-canonical frame-length prefix reproducer",
    },
};
...
test "static_serial malformed frame invariants stay replayable" {
    ...
    const summary = try (Runner{
        .config = .{
            ...
            .run_name = "malformed_frame_invariants",
            .base_seed = .init(0x5173_2026_0320_0001),
            .build_mode = .debug,
        ...
        .persistence = .{
            .io = io,
            .dir = tmp_dir.dir,
            .naming = .{ .prefix = "static_serial_frames" },
...
test "static_serial retained malformed frame bundles preserve replay metadata" {
    ...
    const summary = try (Runner{
        .config = .{
            ...
            .base_seed = .init(0x5173_2026_0320_0002),
            .build_mode = .debug,
            .case_count_max = 8,
        },
        .target = .{
        ...
        .persistence = .{
            .io = io,
            .dir = tmp_dir.dir,
            .naming = .{ .prefix = "static_serial_retained" },
            .artifact_buffer = &artifact_buffer,
            .entry_name_buffer = &entry_name_buffer,
        },
    }).run();
    ...
    const bundle_meta = try failure_bundle.writeFailureBundle(.{
        .io = io,
        .dir = tmp_dir.dir,
        .naming = .{ .prefix = "static_serial_bundle" },
        ...
        .campaign_profile = "malformed_frame_runtime",
...
fn evaluateInvariantCase(seed_value: u64) Evaluation {
    const generated = support.buildGeneratedFrameBytes(seed_value);
    const check = support.evaluateFrameCase(
        generated.bytes[0..generated.len],
        &frame_parse_violation,
    );
    return .{
