...
test "storage durability can snapshot pending operations and stored state for deterministic replay" {
    ...
    try std.testing.expectEqual(durability.CompletionStatus.corrupted, pre_replay_fault_write.status);
    try std.testing.expectEqual(@as(u32, 700), pre_replay_fault_write.value.?);
    ...
    try std.testing.expectEqual(durability.OperationKind.write, replay_write.kind);
    try std.testing.expectEqual(durability.CompletionStatus.success, replay_write.status);
    try std.testing.expectEqual(@as(u32, 222), replay_write.value.?);

    const replay_read = try completions.recv();
    try std.testing.expectEqual(durability.OperationKind.read, replay_read.kind);
    try std.testing.expectEqual(durability.CompletionStatus.success, replay_read.status);
    try std.testing.expectEqual(@as(u32, 700), replay_read.value.?);
    ...
    try std.testing.expect(recover_before_replay_write.check_result.passed);
    ...
    try std.testing.expect(replay_write_before_replay_read.check_result.passed);
}
