...
test "network link can snapshot effective pending deliveries and replay them deterministically" {
    ...
    try testing.expectEqual(@as(u32, 130), try mailbox_13.recv());
    ...
    try testing.expectEqual(@as(u32, 0), early_19.delivered_count);
    ...
    try testing.expectEqual(@as(u32, 1), delivered_19.delivered_count);
    try testing.expectEqual(@as(u32, 190), try mailbox_19.recv());
    ...
    try testing.expect(once_to_19.check_result.passed);
}
