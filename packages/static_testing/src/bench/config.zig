...
comptime {
    ...
    assert(std.meta.fields(BenchmarkMode).len == 2);
}
...
pub fn validateConfig(config: BenchmarkConfig) BenchmarkConfigError!void {
    ...
    assert(measure_runs_total >= config.measure_iterations);
    assert(measure_runs_total >= config.sample_count);
}
...
test "benchmark config defaults are internally consistent" {
    ...
    try testing.expect(BenchmarkConfig.smokeDefaults().sample_count < BenchmarkConfig.fullDefaults().sample_count);
}
...
test "validateConfig rejects multiplication overflow" {
    ...
    if (@sizeOf(usize) == 4) {
        try testing.expectError(error.Overflow, validateConfig(overflow_candidate));
    } else {
        comptime assert(max_product <= std.math.maxInt(usize));
