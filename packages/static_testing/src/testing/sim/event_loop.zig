...
pub const EventLoop = struct {
    ...
    pub fn step(
        ...
        std.debug.assert(consumed_faults.len == due_faults.len);

        const ready_enqueued_count = if (due_timer_count == 0)
            @as(u32, 0)
        else
            try self.timer_queue.drainDue(timer_buffer);
        std.debug.assert(ready_enqueued_count == @as(u32, @intCast(due_timer_count)));
        ...
        if (jump_target_time) |target_time| {
            ...
            std.debug.assert(jumped_time.tick == target_time.tick);
            return .{
                .progress_made = true,
                .time_advanced = true,
                .ready_enqueued_count = ready_enqueued_count,
    ...
    fn nextDueTimeAfterDueFaults(self: *const EventLoop, now_time: clock.LogicalTime) ?clock.LogicalTime {
        ...
        std.debug.assert(next_due_time.?.tick > now_time.tick);
    ...
    if (sim_loop.trace_buffer) |event_trace_buffer| {
        if (sim_loop.scheduler.trace_buffer) |decision_trace_buffer| {
            if (event_trace_buffer == decision_trace_buffer) {
                ...
                std.debug.assert(combined_trace_count >= event_trace_count);
...
test "event loop delivers timers in order and schedules due work" {
    ...
    var timer_queue_value = try timer_queue.TimerQueue(scheduler.ReadyItem).init(std.testing.allocator, &sim_clock, .{
        .buckets = 8,
        .timers_max = 8,
    });
    defer timer_queue_value.deinit(std.testing.allocator);
    ...
    try std.testing.expect((try loop.step(&timer_buffer)).time_advanced);
    const first_ready = try loop.step(&timer_buffer);
    try std.testing.expect(first_ready.decision != null);
    try std.testing.expectEqual(@as(u32, 1), first_ready.decision.?.chosen_id);

    try std.testing.expect((try loop.step(&timer_buffer)).time_advanced);
    const second_ready = try loop.step(&timer_buffer);
    try std.testing.expect(second_ready.decision != null);
    try std.testing.expectEqual(@as(u32, 2), second_ready.decision.?.chosen_id);
}
...
test "event loop stops on explicit step budget" {
    ...
    try std.testing.expectEqual(StopReason.step_budget_exhausted, result.reason);
}
...
test "event loop faults are observational and do not block scheduling" {
    ...
    try std.testing.expectEqual(@as(u32, 1), result.fault_count);
    try std.testing.expect(result.decision != null);
    try std.testing.expectEqual(@as(u32, 17), result.decision.?.chosen_id);
    try std.testing.expectEqual(@as(usize, 1), snapshot.items.len);
    try std.testing.expectEqualStrings("event_loop.fault", snapshot.items[0].label);
}
...
test "event loop runForSteps respects configured step budget" {
    ...
    try std.testing.expectEqual(@as(u32, 1), result.steps_run);
    try std.testing.expectEqual(StopReason.step_budget_exhausted, result.reason);
}
...
test "event loop reports idle when no timers faults or ready work exist" {
    ...
    try std.testing.expectEqual(@as(u32, 0), result.steps_run);
    try std.testing.expectEqual(StopReason.idle, result.reason);
}
...
test "event loop runUntil rejects backward targets" {
    ...
    var timer_buffer: [1]scheduler.ReadyItem = undefined;
    try std.testing.expectError(error.InvalidInput, loop.runUntil(.init(4), &timer_buffer));
}
...
test "event loop traces due faults before jumping to the next timer" {
    ...
    try std.testing.expect(step_result.time_advanced);
    try std.testing.expectEqual(@as(u32, 1), step_result.fault_count);
    try std.testing.expectEqual(@as(usize, 2), snapshot.items.len);
    try std.testing.expectEqualStrings("event_loop.fault", snapshot.items[0].label);
    try std.testing.expectEqualStrings("event_loop.jump", snapshot.items[1].label);
    try std.testing.expectEqual(@as(u64, 2), snapshot.items[1].value);
}
...
test "event loop leaves fault cursors and time unchanged when fault tracing fails" {
    ...
    var timer_buffer: [1]scheduler.ReadyItem = undefined;
    try std.testing.expectError(error.NoSpaceLeft, loop.step(&timer_buffer));
    try std.testing.expectEqual(@as(usize, 0), sim_fault_script.next_index);
    try std.testing.expectEqual(@as(u64, 0), sim_clock.now().tick);

    trace_buffer.reset();
    const recovered = try loop.step(&timer_buffer);
    try std.testing.expectEqual(@as(u32, 1), recovered.fault_count);
    try std.testing.expectEqual(@as(usize, 1), sim_fault_script.next_index);
}
...
test "event loop leaves due timers queued when scheduler capacity is exhausted" {
    ...
    var timer_buffer: [1]scheduler.ReadyItem = undefined;
    try std.testing.expectError(error.NoSpaceLeft, loop.step(&timer_buffer));
    try std.testing.expectEqual(@as(usize, 1), sim_scheduler.readyCount());
    try std.testing.expectEqual(@as(usize, 1), timer_queue_value.dueCountUpTo(sim_clock.now()));

    const manual = try sim_scheduler.nextDecision();
    try std.testing.expectEqual(@as(u32, 7), manual.chosen_id);

    const recovered = try loop.step(&timer_buffer);
    try std.testing.expect(recovered.decision != null);
    try std.testing.expectEqual(@as(u32, 9), recovered.decision.?.chosen_id);
}
...
test "event loop leaves time unchanged when jump tracing lacks capacity" {
    ...
    trace_buffer.reset();
    const recovered = try loop.step(&timer_buffer);
    try std.testing.expect(recovered.time_advanced);
    try std.testing.expectEqual(@as(u64, 2), sim_clock.now().tick);
}
