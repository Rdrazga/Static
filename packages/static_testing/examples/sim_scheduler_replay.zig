...
pub fn main() !void {
    ...
    std.debug.assert(replayed.step_index == recorded.step_index);
    std.debug.assert(replayed.chosen_id == recorded.chosen_id);
    std.debug.assert(replayer.recordedDecisions().len == 1);
    std.debug.assert(replayer.readyCount() == 2);
