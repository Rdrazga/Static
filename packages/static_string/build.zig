...
pub fn build(b: *std.Build) void {
    ...
    options.addOption([]const u8, "static_package", "static_string");
    ...
    const tests = b.addTest(.{ .root_module = static_string_mod });
    ...
    const examples = [_]struct { name: []const u8, path: []const u8 }{
        .{ .name = "static_string_ascii_normalize_basic", .path = "examples/ascii_normalize_basic.zig" },
        .{ .name = "static_string_bounded_buffer_basic", .path = "examples/bounded_buffer_basic.zig" },
        .{ .name = "static_string_intern_pool_basic", .path = "examples/intern_pool_basic.zig" },
        .{ .name = "static_string_utf8_validate_basic", .path = "examples/utf8_validate_basic.zig" },
    };
    for (examples) |ex| {
        const exe = b.addExecutable(.{
            ...
            .root_module = b.createModule(.{
                ...
                .optimize = optimize,
                .imports = &.{
                    .{ .name = "static_string", .module = static_string_mod },
                },
            }),
        });
