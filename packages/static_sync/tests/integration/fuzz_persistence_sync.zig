...
const failure_threshold: u64 = 2048;
const violations = [_]testing.testing.checker.Violation{
    .{ .code = "static_sync.cancel_threshold", .message = "cancel seed reached the deterministic failure threshold" },
};

test "static_sync fuzz persistence keeps a reduced failing seed replayable" {
    ...
    const config = testing.testing.fuzz_runner.FuzzConfig{
        ...
        .run_name = "cancel_reduced_failure",
        .base_seed = .{ .value = 20260317 },
        .build_mode = .debug,
        .case_count_max = 32,
        .reduction_budget = .{
            .max_attempts = 64,
    ...
    const first_failing = findFirstFailingSeed(config).?;

    const Runner = testing.testing.fuzz_runner.FuzzRunner(sync.cancel.RegisterError, error{});
    ...
    const runner = Runner{
        ...
        .persistence = .{
            .io = io,
            .dir = tmp_dir.dir,
            .naming = .{ .prefix = "static_sync_cancel_reduce" },
...
const TargetContext = struct {
    ...
    ) sync.cancel.RegisterError!testing.testing.fuzz_runner.FuzzExecution {
...
const ReplayContext = struct {
    ...
    fn run(
        ...
        const failing = artifact.identity.seed.value >= context.threshold;
        return .{
...
fn findFirstFailingSeed(config: testing.testing.fuzz_runner.FuzzConfig) ?FirstFailingSeed {
