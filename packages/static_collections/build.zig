// This package is built as part of the static workspace. Standalone
// `zig build` from this directory is not supported because the workspace
// root provides shared dependency resolution and build-option propagation
// that individual packages cannot replicate independently.
...
pub fn build(b: *std.Build) void {
    ...
    addExamples(b, target, optimize, static_collections_mod);
}
...
fn addExamples(
    ...
    static_collections_mod: *std.Build.Module,
) void {
    const examples_step = b.step("examples", "Build examples");
    const examples = [_]struct { name: []const u8, path: []const u8 }{
        .{ .name = "static_collections_vec_basic", .path = "examples/vec_basic.zig" },
        .{ .name = "static_collections_flat_hash_map_seeded", .path = "examples/flat_hash_map_seeded.zig" },
        .{ .name = "static_collections_slot_map_handles", .path = "examples/slot_map_handles.zig" },
        .{ .name = "static_collections_min_heap_basic", .path = "examples/min_heap_basic.zig" },
    };
    for (examples) |ex| {
