//! Model-based coverage for FlatHashMap mutation sequences under clustered hashes.
//! Exercises collision-heavy put/get/remove/clear/clone flows against a bounded
//! reference model while using the borrowed callback path.
...
const ClusteredCtx = struct {
    pub fn hash(key: *const Key, seed: u64) u64 {
        return seed ^ (@as(u64, key.bucket) << 32);
    }
...
const Map = static_collections.flat_hash_map.FlatHashMap(Key, u32, ClusteredCtx);
const InitialCapacity: usize = 8;
const ScenarioCount: u32 = 3;
const ActionCount: u32 = 14;

const key_universe = [_]Key{
    .{ .bucket = 1, .id = 10, .tag = 1 },
    .{ .bucket = 1, .id = 20, .tag = 2 },
    .{ .bucket = 1, .id = 30, .tag = 3 },
    .{ .bucket = 1, .id = 40, .tag = 4 },
    .{ .bucket = 1, .id = 50, .tag = 5 },
    .{ .bucket = 1, .id = 60, .tag = 6 },
    .{ .bucket = 2, .id = 70, .tag = 7 },
};
...
const violations = [_]checker.Violation{
    .{
        .code = "static_collections.flat_hash_map_model",
        .message = "flat hash map runtime sequence diverged from the bounded reference model",
    },
};
...
const action_table = [ScenarioCount][ActionCount]model.RecordedAction{
    .{
        ...
        encodeAction(.put, 1, 20),
        encodeAction(.remove, 6, 0),
        encodeAction(.get_or_put, 1, 99),
        encodeAction(.put, 2, 30),
        encodeAction(.put, 3, 40),
        encodeAction(.put, 4, 50),
        encodeAction(.put, 5, 60),
        encodeAction(.probe, 2, 0),
        encodeAction(.remove_or_null, 4, 0),
        encodeAction(.clone_validate, 0, 0),
        encodeAction(.clear, 0, 0),
        encodeAction(.get_or_put, 6, 70),
        encodeAction(.clone_validate, 0, 0),
    },
    .{
        encodeAction(.put, 5, 60),
        ...
        encodeAction(.put, 1, 20),
        encodeAction(.probe, 5, 0),
        encodeAction(.put, 2, 30),
        encodeAction(.remove, 4, 0),
        encodeAction(.get_or_put, 1, 99),
        encodeAction(.put, 3, 40),
        encodeAction(.put, 4, 50),
        encodeAction(.remove_or_null, 5, 0),
        encodeAction(.put, 5, 61),
        encodeAction(.clone_validate, 0, 0),
        encodeAction(.clear, 0, 0),
        encodeAction(.get_or_put, 6, 70),
    },
    .{
        encodeAction(.put, 0, 10),
        encodeAction(.put, 2, 30),
        encodeAction(.remove, 3, 0),
        encodeAction(.get_or_put, 4, 50),
        encodeAction(.put, 1, 20),
        encodeAction(.probe, 4, 0),
        encodeAction(.put, 3, 40),
        encodeAction(.get_or_put, 2, 99),
        encodeAction(.put, 5, 60),
        encodeAction(.remove_or_null, 0, 0),
        encodeAction(.clone_validate, 0, 0),
        encodeAction(.clear, 0, 0),
        encodeAction(.get_or_put, 6, 70),
        encodeAction(.clone_validate, 0, 0),
...
const Context = struct {
    ...
    saw_borrowed_probe: bool = false,
    saw_clear: bool = false,
    saw_clone: bool = false,

    fn resetState(self: *@This()) void {
        if (self.map_initialized) self.map.deinit();
        self.map = Map.init(testing.allocator, .{
            .initial_capacity = InitialCapacity,
            .budget = null,
        }) catch |err| panic("resetState: FlatHashMap.init failed: {s}", .{@errorName(err)});
        ...
        self.saw_borrowed_probe = false;
        self.saw_clear = false;
        self.saw_clone = false;
        assert(self.map.len() == 0);
        assert(self.map.capacity() == InitialCapacity);
    }
    ...
    fn validate(self: *@This()) checker.CheckResult {
        ...
        if (self.map.capacity() > InitialCapacity) self.saw_growth = true;

        var digest: u128 = @as(u128, self.map.capacity()) << 64;
        var slot: usize = 0;
    ...
    fn getOrPutValue(self: *@This(), slot: usize, value: u32) checker.CheckResult {
        ...
        if (result.found_existing != expected_existing) {
            return checker.CheckResult.fail(&violations, null);
        }
    ...
    fn probeValue(self: *@This(), slot: usize) checker.CheckResult {
        ...
        self.saw_borrowed_probe = true;
        return self.validate();
    }

    fn clearMap(self: *@This()) checker.CheckResult {
        const capacity_before = self.map.capacity();
        self.map.clear();
        self.reference.clear();
        if (self.map.capacity() != capacity_before) return checker.CheckResult.fail(&violations, null);
        self.saw_clear = true;
        return self.validate();
    }
    ...
    fn cloneValidate(self: *@This()) checker.CheckResult {
        ...
        if (!compareMapToReference(&clone, &self.reference)) {
            return checker.CheckResult.fail(&violations, null);
        }
    ...
    fn finish(self: *@This()) checker.CheckResult {
        ...
        assert(self.saw_borrowed_probe);
...
test "flat hash map runtime sequences stay aligned with testing.model" {
    ...
    const summary = try model.runModelCases(error{}, Runner{
        .config = .{
            ...
            .run_name = "flat_hash_map_runtime_sequences",
