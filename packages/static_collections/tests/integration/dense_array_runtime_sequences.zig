...
fn expectItemAt(array: *DenseArray, index: usize, expected: u32) !void {
    try testing.expectEqual(expected, array.get(index).?.*);
    try testing.expectEqual(expected, array.getConst(index).?.*);
}

test "dense array append returns stable indices and exposes values through get APIs" {
    var array = try DenseArray.init(testing.allocator, .{ .budget = null });
    defer array.deinit();
    ...
    try testing.expectEqual(@as(usize, 2), third);
    ...
    try testing.expect(array.getConst(3) == null);
}
...
test "dense array swapRemove preserves density and rejects invalid indices" {
    ...
    try testing.expectEqual(@as(u32, 30), last_removed);
    try testing.expectEqual(@as(usize, 2), array.len());
    try expectItemAt(&array, 0, 10);
    try expectItemAt(&array, 1, 40);

    try testing.expectError(error.NotFound, array.swapRemove(2));
    try testing.expectError(error.NotFound, array.swapRemove(99));
}
...
test "dense array itemsConst reflects dense contents after churn" {
    ...
    try testing.expectEqual(@as(u32, 1), before[0]);
    try testing.expectEqual(@as(u32, 2), before[1]);
    try testing.expectEqual(@as(u32, 3), before[2]);
    ...
    try testing.expectEqual(@as(u32, 3), after[0]);
    try testing.expectEqual(@as(u32, 2), after[1]);
}
...
test "dense array capacity and clear lifecycle" {
    ...
    var array = try DenseArray.init(testing.allocator, .{ .initial_capacity = 4, .budget = null });
    defer array.deinit();

    try testing.expect(array.capacity() >= 4);
    ...
    try testing.expect(cap_before >= 3);

    array.clear();
    try testing.expectEqual(@as(usize, 0), array.len());
    try testing.expectEqual(cap_before, array.capacity());
    try testing.expectEqual(@as(usize, 0), array.itemsConst().len);

    // Re-append within existing capacity.
    _ = try array.append(40);
    _ = try array.append(50);
    try testing.expectEqual(@as(usize, 2), array.len());
    try testing.expectEqual(@as(u32, 40), array.itemsConst()[0]);
    try testing.expectEqual(@as(u32, 50), array.itemsConst()[1]);
}
...
test "dense array budget propagation tracks growth and releases on deinit" {
    ...
    {
        var array = try DenseArray.init(testing.allocator, .{
            .budget = &budget,
        });
        defer array.deinit();
        ...
        try testing.expect(used_after_appends > 0);

        // Clear does not release budget (capacity retained).
        array.clear();
        try testing.expectEqual(used_after_appends, budget.used());
    }
