...
pub fn main() !void {
    ...
    std.debug.assert(parent_roll >= 10);
    std.debug.assert(parent_roll <= 20);
    std.debug.assert(child_roll >= 10);
    std.debug.assert(child_roll <= 20);

    const parent_bucket = try rng.distributions.uintBelow(&parent, 8);
    const child_bucket = try rng.distributions.uintBelow(&child, 8);
    std.debug.assert(parent_bucket < 8);
    std.debug.assert(child_bucket < 8);

    const parent_unit32 = rng.distributions.f32Unit(&parent);
    const child_unit64 = rng.distributions.f64Unit(&child);
    std.debug.assert(parent_unit32 >= 0.0);
    std.debug.assert(parent_unit32 < 1.0);
    std.debug.assert(child_unit64 >= 0.0);
    std.debug.assert(child_unit64 < 1.0);
