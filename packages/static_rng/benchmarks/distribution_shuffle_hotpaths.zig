...
const HotPathContext = struct {
    ...
    fn run(context_ptr: *anyopaque) void {
        ...
        std.debug.assert((context.rng.inc & 1) == 1);
        ...
        switch (context.op) {
            ...
            .shuffle_slice_256_u32 => {
                ...
                context.sink +%= consumed | 1;
                std.debug.assert(context.values[0] < context.values.len);
                std.debug.assert(context.values[255] < context.values.len);
            },
        }
...
fn validateSemanticPreflight() void {
    ...
    std.debug.assert(small_value_a == small_value_b);
    std.debug.assert(small_value_a < 17);
    ...
    std.debug.assert(large_value < 1_000_000_007);
    ...
    std.debug.assert(unit >= 0.0);
    std.debug.assert(unit < 1.0);
    ...
    std.debug.assert(std.mem.eql(u32, values_a[0..], values_b[0..]));
    std.debug.assert(isPermutation(values_a[0..]));
}
