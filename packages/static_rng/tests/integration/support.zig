...
pub fn mix64(value: u64) u64 {
    ...
    z ^= z >> 31;
    assert(z != 0 or value == 0);
    assert((z ^ value) != 0 or value == 0);
    return z;
}
...
pub fn foldDigest(digest: u128, value: u64) u128 {
    ...
    assert(result != digest or value == 0);
    assert(@as(u64, @truncate(result)) == next_lower);
    return result;
}

pub fn seedFrom(value: u64, salt: u64) u64 {
    const seed = mix64(value ^ salt);
    assert(seed != 0 or value == 0);
    assert((seed ^ salt) != 0 or value == 0);
    return seed;
}
...
pub fn fillShuffleValues(values: []u32) void {
    assert(values.len == shuffle_len);
    for (values, 0..) |*value, index| {
        value.* = @intCast(index);
    }
    assert(values[0] == 0);
    assert(values[values.len - 1] == @as(u32, @intCast(values.len - 1)));
}
...
pub fn shuffleSlice(rng: anytype, values: []u32) static_rng.distributions.DistributionError!void {
    ...
    assert(values.len == shuffle_len);
    assert(values[0] < @as(u32, @intCast(shuffle_len)));
}
...
pub const ReferencePcg32 = struct {
    ...
    pub fn init(seed: u64, sequence: u64) ReferencePcg32 {
        ...
        assert(result.state != 0 or seed == 0);
        return result;
    }
    ...
    pub fn nextU32(self: *ReferencePcg32) u32 {
        ...
        assert((self.inc & 1) == 1);
        assert(self.state != old_state or self.inc != 1);
        return result;
    }
...
pub const ReferenceSplitMix64 = struct {
    ...
    pub fn init(seed: u64) ReferenceSplitMix64 {
        ...
        assert(result.state == seed);
        assert(result.state ^ seed == 0);
        return result;
    }
    ...
    pub fn next(self: *ReferenceSplitMix64) u64 {
        ...
        z *%= 0x94d0_49bb_1331_11eb;
        z ^= z >> 31;
        assert(self.state == previous_state +% 0x9e37_79b9_7f4a_7c15);
        assert(self.state != previous_state);
        return z;
    }
};
...
pub const ReferenceXoroshiro128Plus = struct {
    ...
    pub fn nextU64(self: *ReferenceXoroshiro128Plus) u64 {
        ...
        assert(result == before.s0 +% before.s1);
        assert(self.s0 != 0 or self.s1 != 0);
        assert(self.s0 != before.s0 or self.s1 != before.s1);
        return result;
    }
